(() => {
  ...
  const open=value=>{panel.classList.toggle('is-open',value);backdrop?.classList.toggle('is-open',value);panel.setAttribute('aria-hidden',String(!value));trigger.setAttribute('aria-expanded',String(value));if(value)localStorage.setItem('sungeum_assistant_opened','1');setMood(value?'listening':'')};
